...
def final_run(X,Y,Xtest):
    ...
    rstate=int(sys.argv[1])
    clf = AdaBoostClassifier(DecisionTreeClassifier(min_samples_split=500,max_depth=5),
            algorithm="SAMME",
            n_estimators=2000,
            random_state=rstate)
    clf.fit(X,Y)
    Ytest=clf.predict(Xtest)
    output(Ytest,'adaboost_about_final_{}.csv'.format(rstate))
...
def cross_val(X,Y):
    ...
    param_grid={
            "n_estimators":[2000],
            "base_estimator__min_samples_split":[500],
            "base_estimator__max_depth":[None,4,5],
            "algorithm":["SAMME","SAMME.R"],
            "learning_rate":[1,0.1]
            }
...
def ada_simple_try(X,Y):
    if sys.argv[1]=='None':
        depth=None
    else:
        depth=int(sys.argv[1])
    clf = AdaBoostClassifier(DecisionTreeClassifier(min_samples_split=500,max_depth=depth),
            algorithm="SAMME",
            n_estimators=2000,random_state=int(sys.argv[2]))
    score =  cross_validation.cross_val_score(clf,X,Y, cv=5, n_jobs=-1)
    print "try ", score," ", score.mean()
    print "clf ", clf
