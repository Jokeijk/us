...
def onescore(X,Y,Xtest):
    clf = RandomForestClassifier(oob_score=True,n_jobs=-1, n_estimators=300, max_features=300,random_state=0)
    clf.fit(X,Y)
    print "oob_score = ",  clf.oob_score_
    print clf.get_params()
    ytest=clf.predict(Xtest)
    output(ytest,"try_005.csv")

def dogridsearch_simple(X,Y,N):
    clf = RandomForestClassifier(oob_score=True,n_jobs=-1, max_features=300,
            n_estimators= N) 
    clf.fit(X,Y)
    print "max_features= {} oob_score ={}".format(N,clf.oob_score_)

X,Y,Xtest=get_data()

#grid_test0 = { "n_estimators"      : [100,200,300,400,500,600,700,800,900],
#               "max_features"      : [300] }
#dogridsearch(X,Y,grid_test0)
#for N in [100,200,300,400,500,600,700,800,900]:
#    dogridsearch_simple(X,Y,N)
